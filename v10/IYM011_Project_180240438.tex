...
Table \ref{table:data_acquisition_lit_review} is a critical review of limitations identified in the literature research and how this project aims to fix them.
...
\hline
\textbf{Section} & \textbf{Limitation} & \textbf{Example(s)} & \textbf{This Projects Reaction} \\
\hline
\endfirsthead
\multicolumn{4}{c}%
...
\hline
\endlastfoot
      \textbf{Filtering data during data acquisition} & If data is pre filtered before the analysis stage it could result in unintended data being filtered & The researches in \cite{Buchanan2018-xz} specify the headers to capture during the data acquisition phase
 & No http headers are filtered out during capture phase \\
      \hline
      \textbf{www sumdomin} & If using base domains as the starting point, not all domains serve http on the base domain. The www subdomain is the most common subdomain to use for the home/default page of a website & Researchers \cite{Buchanan2018-xz,Amann2017-co} chose not to use www & Use www subdomain if base domain does not serve http\\
      \hline
      \textbf{HTTP method} & Browsers use the http GET method to obtain web pages. A website might behave differently if a non GET http method (e.g. HEAD) was used. Some websites may not allow the http HEAD method & Researchers \cite{Amann2017-co} use the HEAD http method & Use the HTTP GET method \\
      \hline
      \textbf{Client Headers} & To best represent a browser, the headers sent on the request to a website should match that of a popular browser or arguably the most popular. The single most relevant header to include is the user agent header. It is quite common when a non browser user agent header is detected a different response is sent. & ?? & Use chrome headers not just the user agent \\
      \hline
      \textbf{SSL/TLS Verification} & If SSL/TLS verification is done at capture time there is a possibility that the client/OS does not necessarily have access to all of the Root/Intermediate certificates that a browser would have & None of the research identified during the lit research mentions this point. There could be many reasons for this such as it not being common knowledge of how browsers cache certificates and or servers may not have up to date root certificate stores & Do not perform the SSL/TLS verification check at the data capture phase. Capture the certificates the website presents and verify SSL/TLS at the analysis phase \\
      \hline
      \textbf{Monitoring} & When a scanning session is conducted and not monitored one or more sessions could fail without the researcher(s) being aware for an unknown amount of time & It is possible that the outage mentioned in \cite{Poteat2021-zr} could have been avoided or minimised by the use of effective monitoring & Generate metrics and alert on them \\
      \hline
      \textbf{Scan frequency} & The more data the better & - & - \\
      \hline
      \textbf{Scanner Tool Verification} & One cannot assume that the technology used to perform the scanning is doing what one expects or intends without due diligence being performed & ?? & Randomly pick good and bad results and verify they are as expected via manually verifying results \\
%       &  &  &  \\
%      \hline
\end{longtable}
\end{center}
