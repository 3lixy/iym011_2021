...
\begin{figure}[p]
	\begin{center}
		\includegraphics[scale=0.5]{../images/HLA_Pre_Implementation_v1.png} 
...
This section describes the chosen technologies and resources for the various aspects for the implementation, showin in Figure \ref{fig:hla_design_implmementation} of the design in section \ref{subsection:hld}.

\begin{figure}[h!]
	\begin{center}
		\includegraphics[scale=0.7]{../images/HLA_Pre_Implementation_v2.png} 
...
\begin{itemize}
    \item Obtains the list of domains from the domain list source (\texttt{https://tranco-list.eu/top-1m.csv.zip})
...
\vspace{0.3cm} \noindent
The research conducted by the Tranco team \cite{Le_Pochat2018-ql}, who provide the Tranco domain list \cite{noauthor_undated-mt},  identified that after comparing several most popular domain lists that that differ on the rankings for the same domain, domains present in the list can vary wildly on a day to day basis and are susceptible to influence by manicous actors.
...
\vspace{0.3cm} \noindent
Below is a High Level flow chart for the process of performing a domain scanning Task.
