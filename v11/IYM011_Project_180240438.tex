...
- Major contributions of research
	- CO** header user
	- TLS versions supported rather than just negotiated
...
\vspace{0.3cm} \noindent
The scanning of websites commenced in November 2020, before the \textit{``Project''} module was officially started in October 2021. This was due to the need to gather at least 12 months of data before the project submission deadline at the end of March 2022.
...
\noindent
The trend for negotiated TLS versions is going in the right direction, with TLS 1.3 being the dominant version as this research began with an upwaord trend and as yet does not show sings of tapering off. TLS negotiations that result in either TLS 1.0 or TLS 1.1 is less than 1\% of all TLS negotiations.
...
\noindent
The ability to utilise a CSP policy has been available since 2015. We are not 7 years on and in January 2022 only 60 thousand domains (6\%) in the top 1 million ranked websites are using a CSP policy to protect their sites. This could indicate that website developers are finding it difficult to implement a CSP policy .
...
- Even though some scans missing largest gap 27 days and even with the missing data trends of use are clear for TLS…

- Critical Analysis
	- Missing scans
	- Retry calls for certain checks e.g. all preload checks. To do in a minimal resource way, enforce a min time between attempts and max attempts
