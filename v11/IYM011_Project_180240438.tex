...
\vspace{0.3cm} \noindent
As the RFC Draft stated that ``\textit{For web-based services, organizations MUST place the "security.txt" file under the "/.well-known/" path}'' e.g. \texttt{https://example.com/.well-known/security.txt} the scanner was initially configured to only try and obtain the security.txt from this location. The statement regarding an exception for legacy devices was was missed when creating the scanner.
...
\vspace{0.3cm} \noindent
Figure \ref{fig:security_txt_unique_vs_domains} shows the number of unique security.txt files and the number of all the security.txt files found on unique domains (seperated by whether they are valid or not). The number of invalid security.txt are a small percentage of all security.txt and the majority of them are html documents unrelated to the security.txt mechanism.
...
\begin{figure}[t]
	\begin{center}
		...
		\caption{unique security.txt (only /.root path) by field use}
...
\vspace{0.3cm} \noindent
Figure \ref{fig:security_txt_by_fields_root_only} \textbf{TODO} hopefully top level domains show same trend if not just say only evaluating well-known as that has the largest longitudinal set of results.
...
\vspace{0.3cm} \noindent
Using the script-src directive to only allow resources to be loaded from trusted sources is a great first step to preventing attacks from loading scripts. The additional use of sub resource integrity can be used to prevent supply chain attacks where the attacker takes control of a trusted source script.
...
\vspace{0.3cm} \noindent
CSP policy maintainers should keep up to date with CSP bypass techniques such as those described in  ``\textit{Complex Security Policy? A Longitudinal Analysis of Deployed Content Security Policies}'' \ref{Roth2020-hg}.
...
\vspace{0.3cm} \noindent
The low amount of sites that are preloaded in the top 1 million most popular domains could be due to that the majority of sites are maintained by individuals with an interest in security and thus not likely to be a very popular domain compared to all the site on the internet.
...
\vspace{0.3cm} \noindent
The usage of the directive \texttt{includeSubDomains} is consistently 40\% of all directives used. Modern websites and applications can have a vast amount of sub-domains and as such will take time to make them all HTTPS if not already. This can be a major factor in the relatively l1	ow use of this directive.
...
\noindent
Figure \ref{fig:hsts_max-age} shows groups of the duration of the \texttt{max-age} directive. The most common duration is 1-3 which is not unsurprising as the RFC states a vlaue of 1 year. The upward trend is in the 1-3 year bracket which indicates that as sites add a STS header they are doing so with at least a year for the \texttt{max-age} directive.
