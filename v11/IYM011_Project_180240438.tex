...
\noindent
Short paragraph leading up to the below question

How far has website security come since Sir Tim Berners-Lee invented the world wide web ?
It is becoming increasingly difficult to go abouts one’s life without the need to access online services such as websites to shop, share our experiences and bank, amongst many others.
In 1995 SSL was proposed [HK01] to make interactions with websites more secure. As time has moved on, more improved secure channel protocols, TLS 1.3 [RE01] being the most recent, have been introduced as well as enhancements to HTTP through security headers such as HTTP Strict Transport Security [HJ01]. 
What improvements have been made since and how widespread is their use ?
...
\noindent
\texttt{What is the current adoption of Security Mechanisms in the HTTPS Ecosystem ?}
...
- security features can turn to do more harm than good
- using security mechanisms for maliciously
...
\textbf{Part I - Security Features Primer} provides a technical overview of the following security features and related technologies, the adoption of which will be analysed later in this report: \textit{HTTP}, \textit{SSL/TLS}, \textit{Same Origin Policy}, \textit{Cross Origin Request Sharing}, \textit{Content-Security-Policy}, \textit{Cross-Origin-Embedder-Policy}, \textit{Cross-Origin-Resource-Policy}, \textit{Corss-Origin-Opener-Policy}, \textit{X-Content-Type-Options}, \textit{X-Frame-Options}, \textit{HTTP-Public-Key-Pins}, \textit{Strict-Transport-Security}, \textit{HSTS Preloading} and \textit{security.txt}
 %\hyperref[section:http]{\textbf{HTTP}}, \hyperref[section:ssl_tls]{\textbf{SSL/TLS}}, \hyperref[section:same_origin_policy]{\textbf{Same Origin Policy}}, \hyperref[section:cors]{\textbf{Cross Origin Request Sharing}}, \hyperref[section:csp]{\textbf{Content-Security-Policy}},  \hyperref[section:coep]{\textbf{Cross-Origin-Embedder-Policy}},  \hyperref[section:corp]{\textbf{Cross-Origin-Resource-Policy}}, \newline \hyperref[section:coop]{\textbf{Corss-Origin-Opener-Policy}}, \hyperref[section:xcto]{\textbf{X-Content-Type-Options}},  \hyperref[section:xfo]{\textbf{X-Frame-Options}}, \hyperref[section:hpkp]{\textbf{HTTP-Public-Key-Pins}}, \hyperref[section:hsts]{\textbf{Strict-Transport-Security}}, \hyperref[section:hsts_preload]{\textbf{HSTS Preloading}} and \hyperref[section:security_txt]{\textbf{security.txt}}
...
\subsection{Website Supported TLS Versions}
...
\noindent
For each website that did support HTTPS, it was also checked to see which of the 4 TLS versions it supported. Figure \ref{fig:tls_verions_supported} shows how many domains supported the 4 TLS versions.
...
\vspace{0.3cm} \noindent
With less than 1\% of negotiated versions utilising either TLS 1.0 or TLS 1.1 perhaps this should be a diver for website maintainers to remove support for them. However as the usage of TLS 1.0 and TLS 1.1 has be very low for many years there may still be a significant enough reason that support for these outdated TLS versions are still in use.

\vspace{0.3cm} \noindent
It is also possible that websites are running old/outdated versions of applications serving these websites and the maintainers are unaware or the updating these older systems are of low priority.
...
\noindent
Figure \ref{fig:hsts_preloaded_overview} shows an overal summary of the current state of top 1 million domains that are in the preload list managed by Google. The number of preloaded domains is quite a low percentage, as of January 2022 only 0.72\% of the top 1 million domains were preloaded. In the most recent HSTS preload list there are 157192 entries which would seem to indicate that most of the preloaded domains are not in the top 1 million most popular.

\vspace{0.3cm} \noindent
As of January 2022 38\% of domains preloaded that do not meet the preload requirements anymore. This is a rather high percentage and these domains are at risk of being remove as per the message dispalyed on hstspreload.org when checking a domain that no longer meets the preload requirements: \textit{``Status: <domain> is currently preloaded, but no longer meets the requirements. It may be at risk of removal.''}. Any data/publications detailing actual removal of domains due to them no longer meeting the requirements was unable to be identified.

\vspace{0.3cm} \noindent
Figure \ref{fig:hsts_preloaded_criteria_not_met} shows, for the domains that no longer meet the preload criteria, the percentage of domains that no longer met each criteria. The most prevalent criteria not met  was the lack of presence of the HSTS header followed by not redirecting from HTTP to HTTPS on the base domain.
...
\vspace{0.3cm} \noindent
It is somewhat puzzling that so many domains no longer meet the criteria. One possible reason is that an issue was identified after the domain had been preloaded and the domain was requested to be removed from the preload list and thus HSTS headers for example were disabled/removed.

\vspace{0.3cm} \noindent
The firefox browser generates their own preload list, https://hg.mozilla.org/mozilla-central/raw-file/tip/security/manager/ssl/nsSTSPreloadList.inc, which is the output of a process, https://hg.mozilla.org/mozilla-central/raw-file/tip/taskcluster/docker/periodic-updates/scripts/getHSTSPreloadList.js, which uses the preload list provided by google and checks each domain before including it, such as to only include domains that still meet the criteria.
