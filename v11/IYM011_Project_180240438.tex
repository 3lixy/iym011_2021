...
\noindent
The world revolves around internet communication and websites are a big part of it. There have been many security improvements to the technology that powers the http ecosystem and as such, analysing how these are or are not being utilised is of great interest and value to the community.
...
\textit{
\begin{enumerate}
  \item The measurements will be restricted to websites as they are interacted with by a large proportion of people today, thus providing a useful analysis target.
...
The Content Security Policy (CSP) mechanism allows restrictions to be specified, in either a header or HTTP meta tag, via the use of a policy.
...
\vspace{0.3cm} \noindent
This mechanism is intended to mitigate against such attacks as clickjacking where an attacker invisibly embeds another site on top of theirs tricking users, via the use of enticing offers such as free electronics, into clicking a harmless link which actually triggers a function on the embedded site such as transferring money via one's banking website to the attacker.
...
\begin{table}[H]
  \begin{center}
    ...
    \begin{tabular}{|c|c|D|E|}  % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      ...
      \hline
      max http/https redirects & int & 10 (golang default) & The number of http redirects allowed \\
      \hline
...
\vspace{0.3cm} \noindent
Appendix TODO details the TLS client capabilities of the Task Agent.
...
%		\def\svgscale{0.4}
        \scalebox{0.47}{\subimport{../images}{task_flow.pdf_tex}}
...
\vspace{0.3cm} \noindent
TLS 1.2 holds fairly steady ranging between 72\% and 75\%. The reason for this is likely due to TLS 1.2 the most recent TLS versions supprted by legacy devices and is likely to remain like this for many years to come. As one might expect, TLS 1.3 is on an upward trend going from 37\% in November 2020 and reaching 48.5\% in January 2022.

\vspace{0.3cm} \noindent
Both TLS 1.0 and TLS 1.1 look to follow the same downward trend with 34.3\% and 36.8\% in January 2022 respectively. As they follow the same trend it is likely due to when libraries and or applications are upgraded rather than manually configuration changes to remove support TLS 1.0 and TLS 1.1.

\vspace{0.3cm} \noindent
For the 0.2\% of websites that negotiate with wither TLS 1.0 or TLS 1.1 they did not support any version higher than they negotiated with. There should be not be a significant reason that TLS 1.2 and TLS 1.3 can be supported whilst also supporting TLS 1.0 and or TLS 1.1 for legacy reasons. 
...
\vspace{0.3cm} \noindent
As the RFC Draft stated that ``\textit{For web-based services, organizations MUST place the "security.txt" file under the "/.well-known/" path}'' e.g. \texttt{https://example.com/.well-known/security.txt} the scanner was initially configured to only try and obtain the security.txt from this location.
...
A parser was created in python that processed security.txt request responses that had HTTP 2xx status codes, split each security.txt on new line terminations and proceeded to process each line individually. Lines that started with a hash "\#" (\%x23) were skipped as these are comments as defined in the draft RFC.
...
\vspace{0.3cm} \noindent
If after processing a security.txt file no fields were detected, the security.txt was not deemed to be valid and thus not included in the data used for analysis.
...
\noindent
Figure \ref{fig:security_txt_unique_vs_domains} shows the number of unique security.txt files and the number of all the security.txt files found on unique domains. The number of unique security.txt files is roughly half of all security.txt files found on unique domains. A reason for this could be that one organisation is responsible for multiple domains and uses a common security.txt for most and or all domains under their control.
...
\begin{figure}[H]
	\begin{center}
        \scalebox{0.90}{\subimport{../images}{security.txt-fields_use_unique_files.pdf_tex}}
		\caption{unique security.txt (only /.well-known path) by field use}
...
\vspace{0.3cm} \noindent
In the most recent preload list commit (Tue Nov 09 22:54:06 2021) there are 34 TLDs configured. This means that any site that is of one of these preload lists is preloaded (i.e. the domain itself is not in the preload list only the TLD)
...
\noindent
Figure \ref{fig:hsts_preloaded_overview} shows an overal summary of the current state of top 1 million domains that are in the preload list managed by Google. The number of preloaded domains is quite a low percentage, as of January 2022 only 0.72\% of the top 1 million domains were preloaded. The most recent HSTS preload list is from December 2021, as of as of January 2022, which contains 157192 (up from 1258 in 2010 \cite{Michael2015-hn}) entries which would seem to indicate that most of the preloaded domains are not in the top 1 million most popular domains.

\vspace{0.3cm} \noindent
The low amount of site that are preloaded in the top 1 million most popular domains could be due to that the majority of sites are maintained by individuals with an interest in security and thus not likely to be a very popular domain compared to all the site on the internet.
...
\vspace{0.3cm} \noindent
If a website is running a HTTP server on its base domain on port 80 it must redirect to must redirect to HTTPS on the same domain. For example of the base domain of \texttt{https://blog.example.com} is \texttt{example.com}. If a http connection was able to be made to \texttt{http://example.com} it must then redirect to \texttt{https://exmaple.com}.
...
\vspace{0.3cm} \noindent
By far the most prevalent reason that a domain no longer met the preload critera was a missing HSTS header. Many websites have numerous subdomains and it can be hard to keep track of all that are in use.Those managing the security aspect may not be aware of all the subdomains and by preloading the base domain caused would have caused issues if one or more subdomains were configured with only HTTP or invalid HTTPS configurations. This would lead to the request to have the base domain removed from the preload list.

\vspace{0.3cm} \noindent
\textit{- Max Age too short}

\vspace{0.3cm} \noindent
4.2\% of preloaded domains that presented a HSTS header, have a \texttt{max-age} directive value of less than one year (31,536,000 seconds). This also happens to be the value given in the HSTS rfc \cite{Hodges2012-pe}.
...
\vspace{0.3cm} \noindent
1.6\% of preloaded domains did not provide the directive \texttt{preload} in the HSTS header. As this directive is only used for the purpose of preloading, one might assume that this was done on purpose such that preloading was not longer desired.
...
\noindent
Figure \ref{fig:hsts_overview} shows the usage of the HSTS header and its directives \texttt{max-age},  \texttt{includeSubDomains} and \texttt{preload}. It is difficult to see, however the usage of the directive \texttt{max-age} is almost constantly present in every HSTS header.

\vspace{0.3cm} \noindent
The usage of the directive \texttt{includeSubDomains} is consistently 40\% of all directives used. Modern websites and applications can have a vast amount of sub-domains which and as such will take time to make them all HTTPS if not already. This can be a major factor in the relatively low use of this directive.
...
\noindent
Figure \ref{fig:hsts_max-age} shows groups of the duration of the \texttt{max-age} directive. The most common duration is 3-5 years which shows confidence that website maintainers are intending to keep their websites HTTPS for the foreseeable future.

\vspace{0.3cm} \noindent
Interestingly both the ranged 0-3 Months and 1-3 years have very similar usage.

\vspace{0.3cm} \noindent
\textbf{TODO very short max-age undermines the purpose of HSTS as users may not visit before the max-age has expired}

\vspace{0.3cm} \noindent
\textbf{TODO max-age 0 opt out number of domains}
...
\noindent
Figure \ref{fig:coop_overview} shows the usage of the COOP header and its directives \texttt{unsafe-none}, \newline \texttt{same-origin-allow-popups} and \texttt{same-origin}. The usage of the COOP header almost double that of the CORP header and quadruple that of the COEP header in January 2022, but is still rather low.

\vspace{0.3cm} \noindent
The most common directive used by far is \texttt{same-origin}, with 84\% of all directives (1029 domains) in January 2022, which the most restrictive of the available directives which is great to see. It is of no real surprise that this is the case and it is relatively safe to assume that most websites would not want isolation between windows or tabs as most functionality required would be within the same tab/window.

\vspace{0.3cm} \noindent
The \texttt{unsafe-none} directive has the second most usage being 193 domains and 15\% of all directives used in January 2022. Lastly the \texttt{same-origin-allow-popups} directive is hardly used with ?? domains and ??\% of all directives used in January 2022.
...
\noindent
Figure \ref{fig:xcto_overview} shows the usage of the XCTO header and its single directive \texttt{nosniff}. As there is only one directive for XCTO, the number domains using it and those using the directive \texttt{nosniff} are identical.
...
\noindent
Figure \ref{fig:xfo_overview} shows the usage of the XCTO header and its single directive \texttt{nosniff}. As there is only one directive for XCTO, the number domains using it and those using the directive \texttt{nosniff} are identical.
...
\footnotesize
  \begin{center}
    \begin{tabular}{|D|D|c|c|}  % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      \hline
    ...
    \end{tabular}
    \caption{Number of Domains using the HPKP header}
    \label{table:hpkp_use} % label MUST be after caption https://stackoverflow.com/questions/1353120/referring-to-a-table-in-latex
  \end{center}
\end{table}
