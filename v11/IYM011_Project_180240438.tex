...
\noindent
Figure \ref{fig:security_txt_by_when_expires} show there are a minimal amount of expired security.txt files (ranging from 0\% to ~3\%). The number of security.txt files that expire in less than a year is trending down and those that are to expire greater than an year is increasing. This looks to indicate that expires values are being changed to last more than one year against the recommendation of the RFC, which is to have an expire value less than a year to avoid staleness.
...
%\tiny
  \begin{center}
    \begin{tabular}{|c|c|c|}  % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      ...
      \hline
      Signature & ~4.5\% & Link to signature file\\
      \hline
...
A parser was created in python which processed the value for a content security header. The parser checked all of the below headers of a scan result for the presence of a CSP header.
...
\vspace{0.3cm} \noindent
Each group shows a steady rise in the use of a CSP headers which is an encouraging sign. There is a long way to go however to have the majority of sites using a CSP policy.
