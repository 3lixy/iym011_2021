...
\clearpage
\section{Security.txt}
The ``security.txt'' mechanism is to aid in informing security researchers how to disclose security issues found on systems such as a website \cite{Foudil2021-vh}. Further details are in the in background portion of this report in section \ref{section:security_txt}.
...
\vspace{0.3cm} \noindent
All non comment lines that were not able to have a file name determined were not processed any further and checked once all security.txt files had been processed. If any of the apparent non RFC format adhering lines were found to have been skipped, when they should not have been, the logic parser logic was analysed and corrected as needed until there we only non conformant lines skipped.
...
Figure \ref{fig:security_txt_by_rank} shows the percentage of websites that the scanner was able to identify having a valid security.txt at the path \texttt{/.well-known/security.txt} e.g. \newline \texttt{https://example.com/.well-known/security.txt}. The line chart is splint by rank groups using the rank provided in the domains list file from traco-list.eu for that day.

\vspace{0.3cm} \noindent
Figure \ref{fig:security_txt_by_rank} closly matches that of a similar chart in \cite{Poteat2021-zr}, however with a overall reduced percentage.
...
\begin{figure}[H]
	\begin{center}
        \scalebox{0.90}{\subimport{../images}{security.txt-by_rank-adjusted.pdf_tex}}
		\caption{security.txt (only /.well-known path) by rank - Adjusted for the potential shortfall of 18\% security.txt}
		\label{fig:security_txt_by_rank_adjusted}
	\end{center}
\end{figure}

\noindent
Figure \ref{fig:security_txt_by_rank_adjusted} shows the data used for figure \ref{fig:security_txt_by_rank} adjusted for the 18\% more security.txt which much more closely match the related chart in \cite{Poteat2021-zr} which in turn indicates that they validate one another's results. In hindsight the scanner should have have also attempted to obtain the security.txt from the top level path from the beginning. 
...
There are several defined fields in the RFC:
\begin{itemize}
	\setlength\itemsep{0.1em}
    ...
    \item Contact - A method by which to use for reporting security vulnerabilities (e.g. email, phone and website urls).
    \item Encryption - States or links to how to encrypt communication.
    \item Expires - The datetime after which that data present in the security.txt should not be used.
...
\noindent
\textbf{TODO some or all of the above state "MUST NOT appear more than once". Make a point to detail which have and how many times}
