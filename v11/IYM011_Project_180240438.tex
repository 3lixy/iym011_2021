...
\vspace{0.3cm} \noindent
Each rank group shows a steady rise in the use of a CSP headers which is an encouraging sign. There is a long way to go however to have the majority of sites using a CSP policy.
...
%\footnotesize
  \begin{center}
    \begin{tabular}{|l|D|c|}  % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      ...
      \texttt{unsafe-eval} & ? & ?\\
      \hline
      \texttt{unsafe-hashes} & N/A & ?\\
      \hline
    \end{tabular}
    \caption{Keywords in default-source directive}
    \label{table:csp_header_count} % label MUST be after caption https://stackoverflow.com/questions/1353120/referring-to-a-table-in-latex
  \end{center}
\end{table}
